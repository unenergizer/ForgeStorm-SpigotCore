...
public class CraftingMenu extends Menu {
	...
	protected void makeMenuItems() {
		...
		for (int i = 0; i < fillBoard.length; i++) {
			switch (fillBoard[i]) {
			case "green": setItem(create, i, new TryCraftingRecipe(PLUGIN, successRecipe)); break;
			case "filler": setItem(filler, i); break;
			case "book": setItem(instructions, i); break;
			case "red": setItem(redPanel, i); break;
			case "purple": setItem(purplePanel, i); break;
			case "exit": setItem(exit, i, new Exit(PLUGIN)); break;
			}	
		}
